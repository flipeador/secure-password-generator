...
function drawSVG(svg, model, options) {
    ...
    for (let x = 0; x < model.count; x++)
        for (let y = 0; y < model.count; y++)
            model.isDark(y, x) ?
            addRect(fragment, x, y, options.color) :
            addRect(fragment, x, y, options.background);

    svg.replaceChildren(fragment);
    svg.setAttribute('viewBox', `0 0 ${model.count} ${model.count}`);
...
function drawCanvas(canvas, model, options) {
    ...
    canvas.width = model.count * size;
    canvas.height = model.count * size;
    ...
    for (let x = 0; x < model.count; x++)
        for (let y = 0; y < model.count; y++)
            model.isDark(y, x) ?
            fillRect(ctx, x, y, size, options.color) :
            fillRect(ctx, x, y, size, options.background);
